...
const emptyWorkOrder: Omit<WorkOrder, "id"> = {
  type: "installation", billboardId: "", billboardCode: "", clientName: "", assignee: "",
  status: "pending", dueDate: new Date().toISOString().split("T")[0], slaHours: 48,
  checklist: [{ item: "Item 1", done: false }], photosBefore: [], photosAfter: [],
};

function WorkOrderForm({ initial, billboards, onSave, onCancel }: {
  initial: Omit<WorkOrder, "id"> & { id?: string }; billboards: { id: string; code: string }[]; onSave: (d: any) => void; onCancel: () => void;
}) {
    ...
    <div className="fixed inset-0 bg-background/85 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={onCancel}>
      <div className="glass-panel max-w-md w-full animate-slide-up" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 space-y-3">
          <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">Tipo</label><select className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none" value={form.type} onChange={e => set("type", e.target.value)}>
            {Object.entries(typeLabels).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select></div>
          <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">Ponto</label><select className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none" value={form.billboardId} onChange={e => { const b = billboards.find(x => x.id === e.target.value); set("billboardId", e.target.value); set("billboardCode", b?.code || ""); }}>
            <option value="">Selecione...</option>
            {billboards.map(b => <option key={b.id} value={b.id}>#{b.code}</option>)}
          </select></div>
          <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">Cliente</label><input className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" value={form.clientName} onChange={e => set("clientName", e.target.value)} /></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">Prazo</label><input type="date" className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" value={form.dueDate} onChange={e => set("dueDate", e.target.value)} /></div>
            <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">SLA (horas)</label><input type="number" className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-primary" value={form.slaHours} onChange={e => set("slaHours", parseInt(e.target.value) || 0)} /></div>
          </div>
          <div><label className="text-[10px] uppercase tracking-wider text-muted-foreground">Status</label><select className="w-full bg-muted rounded-lg px-3 py-2 text-sm outline-none" value={form.status} onChange={e => set("status", e.target.value)}>
...
export default function Operations() {
  ...
  const handleSave = (data: any) => {
    if (data.id) {
      ...
      addWorkOrder({ ...data, id: `os${Date.now()}` });
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1200px]">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => { setEditingOS({ ...emptyWorkOrder }); setFormOpen(true); }}>
          ...
          return (
            <div key={os.id} className="stat-card">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">#{os.billboardCode}{os.clientName && ` · ${os.clientName}`}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between mt-4 pt-3 border-t border-border text-xs text-muted-foreground">
                ...
                <span className={os.status === "overdue" ? "text-destructive font-semibold" : ""}>Prazo: {new Date(os.dueDate).toLocaleDateString("pt-BR")}</span>
              </div>
            </div>
          );
        })}
